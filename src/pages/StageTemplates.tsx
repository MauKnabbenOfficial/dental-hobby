...
export default function StageTemplates() {
  const { stageTemplates, addStageTemplate, updateStageTemplate, deleteStageTemplate, generateId } = useData();
  ...
  const resetForm = () => {
    setFormData({ name: '', description: '', defaultDuration: '', checklistItems: [] });
    setNewChecklistItem('');
  ...
  return (
    <div className="space-y-6 animate-slide-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Modelos de Etapas</h1>
          <p className="text-muted-foreground">Cadastre etapas reutilizáveis para procedimentos</p>
        </div>
        <Dialog open={isFormOpen} onOpenChange={(open) => { setIsFormOpen(open); if (!open) resetForm(); }}>
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>{editingTemplate ? 'Editar' : 'Cadastrar'} Modelo de Etapa</DialogTitle>
            </DialogHeader>
              ...
              {template.checklistItems.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  ...
                  {template.checklistItems.length > 3 && (
                    <Badge variant="outline" className="text-xs">+{template.checklistItems.length - 3}</Badge>
                  )}
                </div>
              )}
            </CardContent>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir este modelo de etapa? Esta ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
